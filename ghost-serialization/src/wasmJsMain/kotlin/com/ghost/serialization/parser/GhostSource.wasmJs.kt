...
class WasmByteArraySource(val data: ByteArray) : GhostSource {
    ...
    override fun scanString(start: Int, limit: Int, reader: GhostJsonReader): Int {
        ...
        while (pos < limit) {
            val b = d[pos]
            if (b == GhostJsonConstants.QUOTE_BYTE) { reader.position = pos; return hash }
            if (b == GhostJsonConstants.BACKSLASH_BYTE || b in GhostJsonConstants.CONTROL_CHAR_START..GhostJsonConstants.CONTROL_CHAR_LIMIT) return -1
...
class WasmJsSource(val data: JsAny) : GhostSource {
    ...
    override fun scanString(start: Int, limit: Int, reader: GhostJsonReader): Int {
        val quotePos = findClosingQuote(start, limit)
        if (quotePos != -1) {
            reader.position = quotePos
            return calculateHash(start, quotePos - start)
        }
        return -1
    }
