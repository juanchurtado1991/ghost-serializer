...
object GhostYamlBenchmark {
    ...
    /**
     * Reads @SerialName and @GhostName annotations to resolve the YAML field name,
     * so Jackson can parse YAML generated by KAML or Ghost without any changes to
     * the shared model classes and without adding @JsonProperty to every field.
     */
    private class SerialNameAnnotationIntrospector : NopAnnotationIntrospector() {
        override fun findNameForDeserialization(a: Annotated): PropertyName? {
            val serialName = a.getAnnotation(SerialName::class.java)
            if (serialName != null) return PropertyName.construct(serialName.value)
            val ghostName = a.getAnnotation(GhostName::class.java)
            if (ghostName != null) return PropertyName.construct(ghostName.name)
