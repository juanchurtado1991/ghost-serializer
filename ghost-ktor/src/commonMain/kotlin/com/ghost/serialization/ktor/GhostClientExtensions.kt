...
@PublishedApi
internal const val CLIENT_ERROR_SUFFIX = ". Make sure it is annotated with @GhostSerializable."
...
suspend inline fun <reified T : Any> HttpResponse.bodyGhost(): T {
    ...
    val serializer = Ghost.getSerializer(T::class as KClass<Any>)
        ?: throw IllegalArgumentException("$CLIENT_ERROR_PREFIX${T::class.simpleName}$CLIENT_ERROR_SUFFIX")
    return Ghost.deserialize(serializer, bytes) as T
}
