...
- **`@GhostProtoSerialization` annotation** (`ghost-api`): Proto3-JSON-mapping variant of `@GhostSerialization` for direct top-level properties — lowerCamelCase field names, default/empty values omitted on serialize, 64-bit integers (`Long`) as quoted strings, enums as strings (already Ghost's default), `ByteArray` as Base64. `GhostSerializationProcessor` scans for both annotations; `GhostAnalyzer` auto-converts `snake_case` field names to `lowerCamelCase` for `@GhostProtoSerialization` classes (unless an explicit `@GhostName`/`@SerialName` overrides it) and recognizes `@GhostProtoSerialization` types as valid nested Ghost types alongside `@GhostSerialization`. Not yet covered: `Long`/`ByteArray` reached through a value class or as `List`/`Map` elements, and `oneof`/sealed-class mapping — see the wiki page's "Known gaps" section.
...
- **Test coverage**: `ProtoJsonNumericTest`, `ProtoValueTest`, `ProtoWktTest`, `ProtoWktStructsTest`, `ProtoWktEdgeCasesTest`, `ProtoWrappersTest`, `ProtoAnyTest` (`ghost-protobuf/src/commonTest`) and `GhostProtoSerializationKspTest` (`ghost-compiler/src/test`, generated-code assertions for int64 quoting, Base64 `ByteArray` handling, and zero-value omission).

### Fixed
- **`ghost-protobuf` tests were never running in CI**: `:ghost-protobuf:jvmTest` was missing from `ciTestJvmModules` in the root `build.gradle.kts`, so `./gradlew ciTestJvm`/`ciTest`/`allTests` and the GitHub Actions `test-jvm` job silently skipped the entire module. Added to the gate.
...
### Known limitations
- **No `oneof` mapping**: `@GhostProtoSerialization` has no `discriminator` parameter; a sealed class would fall back to Ghost's generic `"type"`-discriminator wrapping, which doesn't match proto3's `oneof` JSON representation.
- **`uint64`/`fixed64` full range not representable**: mapped onto signed Kotlin `Long`, so legitimate `uint64` values above `Long.MAX_VALUE` can't round-trip. No `ULong` mapping exists.
- **`ProtoAny` has no type registry**: `typeUrl` → concrete Kotlin type resolution (`pack`/`unpack`) isn't implemented; callers must decode the captured `value` bytes manually (e.g. via `RawJson`/`decodeAs<T>()`).
- **No Retrofit/Ktor/Spring Boot integration**: no proto3-JSON `Converter.Factory`/`ContentConverter`/`HttpMessageConverter` — wire `GhostProtobuf.deserialize`/`Ghost.encodeToBytes` in manually. `ghost-gradle-plugin` also doesn't auto-inject `:ghost-protobuf` the way it does for ktor/retrofit.
- **Proto codegen scope**: the `Long`-quoting/`ByteArray`-Base64/default-omission codegen fixes above only cover properties declared directly on the annotated class — not `Long`/`ByteArray` reached through a `@JvmInline value class` wrapper or as `List`/`Map` elements.
