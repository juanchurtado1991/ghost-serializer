...
actual class YamlBenchmarkRepository actual constructor() {
    ...
    actual suspend fun runBenchmark(
        ...
        try {
            ...
            onStatusChange("Benchmarking [ENCODE_STRING] JACKSON...")
            val encodeStringJackson = measureEngine("[ENCODE_STRING] JACKSON") {
                jacksonYaml.writeValueAsString(sampleData)
            }
            ...
            onStatusChange("Benchmarking [ENCODE_BYTES] JACKSON...")
            val encodeBytesJackson = measureEngine("[ENCODE_BYTES] JACKSON") {
                jacksonYaml.writeValueAsBytes(sampleData)
            }
            ...
            onStatusChange("Benchmarking [DECODE_STRING] JACKSON...")
            val decodeStringJackson = measureEngine("[DECODE_STRING] JACKSON") {
                jacksonYaml.readValue<ComplexResponse>(ghostYamlString)
            }
            ...
            onStatusChange("Benchmarking [DECODE_BYTES] JACKSON...")
            val decodeBytesJackson = measureEngine("[DECODE_BYTES] JACKSON") {
                jacksonYaml.readValue<ComplexResponse>(ghostYamlBytes)
            }

            onStatusChange("Done!")

            val engineResults = listOf(
                encodeStringGhost, encodeStringKaml, encodeStringJackson,
                encodeBytesGhost, encodeBytesKaml, encodeBytesJackson,
                decodeStringGhost, decodeStringKaml, decodeStringJackson,
                decodeBytesGhost, decodeBytesKaml, decodeBytesJackson
            )
