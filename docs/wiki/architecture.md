...
2. **`GhostJsonStringReader` (Text-first)**: Parses string inputs natively using `CharArray` scans. Generated by default (`textChannel = true`); byte-only models can opt out with `textChannel = false` and fall back to a one-time UTF-8 bridge into `GhostJsonFlatReader`.
